...
The diagrams below are **generated by this skill itself** — raw SVG following the dark-theme flat-design system. No image generation, no external assets; just hand-authored SVG constrained by the design rules.

### Claude Code Architecture

A structural diagram with three nested subsystems — Interface Layer, Agent Engine, and Tools & API — each containing four components, with bidirectional arrows showing the request/response flow.

![Claude Code Architecture](assets/example-architecture.svg)

*Dark theme · c-gray outer + c-teal + c-amber ramps · 2 nesting levels · viewBox `0 0 680 500`*

### Visualizer Skill Pipeline

A horizontal flowchart showing the four-stage pipeline the skill runs through for every request.
...
├── assets/
│   ├── example-architecture.svg      # Dark-theme Claude Code architecture (structural, 3 subsystems)
